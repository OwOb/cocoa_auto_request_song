...
def request(s) :
    while (1) : #重複嘗試
        try :
            r = requests.get('http://acgmusic.hopto.org:4095/'+s, timeout=3)
            return r
        except :
...
def updata() :
    ...
    r = request('ajax/zh_status.html') #獲取songList
    ...
    for songListName in songListNameList : #依序搜尋songList
        ...
        songId, page = 0, 0
        while 1 : #持續換page搜尋
            r = request('ajax/zh_songlist.html?list_id='+str(songListId)+'&page='+str(page))
            soup = BeautifulSoup(r.text, 'html.parser')
...
while 1 :
    ...
    request('played.html?cmd=request&list_id='+str(requestSongId[0])+'&song_id='+str(requestSongId[1]))
    r = request('ajax/zh_status.html')
    soup = BeautifulSoup(r.text, 'html.parser')
